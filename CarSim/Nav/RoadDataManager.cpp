...
vector<shared_ptr<RoadEdge>> RoadDataManager::FindPath(const shared_ptr<RoadNode> &startNode, const shared_ptr<RoadNode> &destNode) const
{
    ...
    map<int, pair<shared_ptr<RoadNode>, shared_ptr<RoadEdge>>> parent;
    ...
    while (!openList.empty())
    {
        ...
        {
            vector<shared_ptr<RoadEdge>> path;
            for (int nodeId = current->id; nodeId != startNode->id;)
            {
                const auto &[prevNode, edge] = parent[nodeId];
                path.push_back(edge);
                nodeId = prevNode->id;
            }
        ...
        for (const shared_ptr<RoadEdge> &edge : current->edges)
        {
            ...
            {
                {
                    parent[neighborNode->id] = {current, edge};
