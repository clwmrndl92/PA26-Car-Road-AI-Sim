...
struct RWResource
{
    ...
    bool enableCounter;
    bool firstInit;     // Prevent redundant zeroing
};
...
// Constant buffer data for internal use
struct CBufferData
{
    BOOL isDirty = false;
...
struct EffectPass : public IEffectPass
{
    ...
    // Shader parameter constant buffer data (no constant buffer object created)
...
public:
    ...
    // Update and collect shader reflection information
    ...
    void Clear();
    // Create a shader from a blob and assign it an identifier name
    HRESULT CreateShaderFromBlob(std::string_view name, ID3D11Device* device, uint32_t shaderFlag,
        ID3DBlob* blob);

public:
    std::unordered_map<size_t, std::shared_ptr<EffectPass>> m_EffectPasses;			// Render passes

    std::unordered_map<size_t, std::shared_ptr<ConstantBufferVariable>> m_ConstantBufferVariables;  // Constant buffer variables
    std::unordered_map<uint32_t, CBufferData> m_CBuffers;											    // Temporarily cached constant buffer data
    std::unordered_map<uint32_t, ShaderResource> m_ShaderResources;									    // Shader resources
    std::unordered_map<uint32_t, SamplerState> m_Samplers;											    // Samplers
    std::unordered_map<uint32_t, RWResource> m_RWResources;											    // Read-write resources

    std::unordered_map<size_t, std::shared_ptr<VertexShaderInfo>> m_VertexShaders;	// Vertex shaders
    std::unordered_map<size_t, std::shared_ptr<HullShaderInfo>> m_HullShaders;		// Hull shaders
    std::unordered_map<size_t, std::shared_ptr<DomainShaderInfo>> m_DomainShaders;	// Domain shaders
    std::unordered_map<size_t, std::shared_ptr<GeometryShaderInfo>> m_GeometryShaders;// Geometry shaders
    std::unordered_map<size_t, std::shared_ptr<PixelShaderInfo>> m_PixelShaders;		// Pixel shaders
    std::unordered_map<size_t, std::shared_ptr<ComputeShaderInfo>> m_ComputeShaders;	// Compute shaders

    std::filesystem::path m_CacheDir;         // Cache directory path
    bool m_ForceWrite = false;      // Force cache after compilation
};
...
HRESULT EffectHelper::Impl::UpdateShaderReflection(std::string_view name, ID3D11Device* device, ID3D11ShaderReflection* pShaderReflection, uint32_t shaderFlag)
{
    ...
    for (uint32_t i = 0;; ++i)
    {
        ...
        // This will fail after all variables are read, but that is not an error
        if (FAILED(hr))
            break;
        ...
        if (sibDesc.Type == D3D_SIT_CBUFFER)
        {
            ...
            // Get the variable information inside the cbuffer and build the mapping
            ...
            // Determine where the constant buffer should be created
            if (!isParam)
            {
                ...
                // It is possible for cbuffer sizes to differ across shaders; the largest size should be used
                // For example, the current shader may enable the last cbuffer variable via a macro, adding 16 bytes, while another shader disables it
                ...
                // Mark that this shader uses the current constant buffer
            ...
            for (uint32_t j = 0; j < cbDesc.Variables; ++j)
            {
                ...
                // Shader parameters need special handling
                // Record shader uniform parameters
                // **Ignore shader parameter default values**
                if (isParam)
                {
                    switch (shaderFlag)
                    {
                ...
                else
                {
                    ...
                    // If a default value exists, initialize it
            ...
            sibDesc.Type == D3D_SIT_TBUFFER)
        {
            ...
            // Mark that this shader uses the current shader resource
            switch (shaderFlag)
            {
        ...
        else if (sibDesc.Type == D3D_SIT_SAMPLER)
        {
            ...
            // Mark that this shader uses the current sampler
            switch (shaderFlag)
            {
            ...
            sibDesc.Type == D3D_SIT_UAV_CONSUME_STRUCTURED || sibDesc.Type == D3D_SIT_UAV_RWBYTEADDRESS)
        {
            ...
            // Mark that this shader uses the current read-write resource
            switch (shaderFlag)
            {
    ...
    ID3D11Device* device, LPCSTR entryPoint, LPCSTR shaderModel, const D3D_SHADER_MACRO* pDefines, ID3DBlob** ppShaderByteCode)
{
    ...
    // If the shader bytecode cache directory is set and force-write is off, first try to read ${cacheDir}/${shaderName}.cso and add it
    ...
    // If caching is not enabled or there is no cached file, read filename. If it is already bytecode, add it directly
    // Compiled DXBC file header
    ...
    {
        // If filename is HLSL source code, compile and add it. If caching is enabled, the bytecode is saved to ${cacheDir}/${shaderName}.cso

        uint32_t dwShaderFlags = D3DCOMPILE_ENABLE_STRICTNESS;
#ifdef _DEBUG
        // Set the D3DCOMPILE_DEBUG flag to get shader debug info. This flag improves the debugging
        // experience while still allowing shader optimizations
        dwShaderFlags |= D3DCOMPILE_DEBUG;

        // Disable optimizations in Debug builds to avoid certain unexpected behaviors
...
{
    //
    // Set shaders, constant buffers, parameter constant buffers, samplers, shader resources, and read-write resources
    //
    if (pVSInfo)
    {
    ...
    if (pPSInfo)
    {
        ...
        if (pPSInfo->rwUseMask)
        {
            ...
            // Must be set all at once; if any resource needs its counter initialized, all will be initialized
